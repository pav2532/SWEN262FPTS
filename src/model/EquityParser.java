  	...
  	EquitiesHolder temp = new EquitiesHolder();
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				String tickerValue = separateValue[0].replaceAll("\\^[(A-Z)]", "");
				if(separateValue.length == 5){
					newEquity = new Holding(tickerValue.replaceAll("[^a-zA-Z0-9.]", ""),separateValue[1], Float.valueOf(separateValue[2].replaceAll("[^0-9.]", "")), (separateValue[3]+ ", " + separateValue[4].replaceAll("[^a-zA-Z0-9.]", "")));
				}else{
					newEquity = new Holding(tickerValue.replaceAll("[^a-zA-Z0-9.]", ""),separateValue[1], Float.valueOf(separateValue[2].replaceAll("[^0-9.]", "")), separateValue[3].replaceAll("[^a-zA-Z0-9.]", ""));	
				}
