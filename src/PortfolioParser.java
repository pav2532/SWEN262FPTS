			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				if (separateValue[0].equals("B")) {
					...
					if (separateValue[1].equals("S")) {
						sellTransaction sell = new sellTransaction(separateValue[2], separateValue[3],
								Float.valueOf(separateValue[4]), Integer.valueOf(separateValue[5]));
						allTransaction.add(sell);
					} else if (separateValue[1].equals("T")) {
						transferTransaction transfer = new transferTransaction(separateValue[2], separateValue[3],
								Float.valueOf(separateValue[4]));
						allTransaction.add(transfer);
					} else {
						buyTransaction buy = new buyTransaction(separateValue[2], separateValue[3],
								Float.valueOf(separateValue[4]), Integer.valueOf(separateValue[5]));
