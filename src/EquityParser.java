	...
	public EquitiesHolder findAccount(String filename) throws IOException {
		...
		while ((line = br.readLine()) != null) {
			...
			if (separateValue.length == 5)
				newEquity = new Holding(separateValue[0].replaceAll("[^a-zA-Z0-9.]", ""), separateValue[1],
						Float.valueOf(separateValue[2].replaceAll("[^0-9.]", "")),
						(separateValue[3] + ", " + separateValue[4].replaceAll("[^a-zA-Z0-9.]", "")));
			else
				newEquity = new Holding(separateValue[0].replaceAll("[^a-zA-Z0-9.]", ""), separateValue[1],
						Float.valueOf(separateValue[2].replaceAll("[^0-9.]", "")),
						separateValue[3].replaceAll("[^a-zA-Z0-9.]", ""));
